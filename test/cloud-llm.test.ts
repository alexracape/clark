...
describe("CloudLLMProvider", () => {
  ...
  test("preserves underlying provider name", () => {
    const provider = new CloudLLMProvider(
      "https://example.com", "secret", "client-1", "anthropic", "claude-sonnet-4-6",
    );
  ...
  test("sends correct headers and body", async () => {
    ...
    const provider = new CloudLLMProvider(
      "https://api.clark.dev", "my-secret", "uuid-123", "openai", "gpt-4.1-mini",
    );
    ...
    expect(capturedRequest!.headers["Authorization"]).toBe("Bearer my-secret");
